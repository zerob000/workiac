...
colls = ['Mindville','MariaDB','Apache','Hyperledger','IntelDAOS','JFrog','Jira','JiraEcosystem','Mojang','MongoDB','Qt','RedHat','Sakai','SecondLife','Sonatype','Spring']
for coll in colls: 
    ...
    for i in range(len(data)):
        item = data[i]
        # Replace Created Dates with non-timezone aware formats for consistency with other scripts 
        date = datetime.strptime(item[7], date_format)
        date = date.replace(tzinfo=None) # remove timezone awareness as earlier runs did not use this.
        item[7] = date.replace(microsecond = 0) # remove microsecond
        # Replace Resolved Dates with non-timezone aware formats for consistency with other scripts 
        if item[8] != '':
            date = datetime.strptime(item[8], date_format)
            date = date.replace(tzinfo=None) # remove timezone awareness as earlier runs did not use this.
            item[8] = date.replace(microsecond = 0) # remove microsecond
        # Filter and append 
        if item[4] in ('false'): # remove subtasks
            if item[3] not in ('Epic'):
                if item[2] in JiraProjects:
                    JiraProjects[item[2]].append(item)
                else:
                    JiraProjects[item[2]] = [item]
            else:
    ...
    for sname, details in JiraProjects.items():
        ...
        if len(details) >= 30: # remove systems with fewer than 30 PBIs
            ...
            csv_F.write("Key,ID,Project,IssueType,IsSubtask,Status,Resolution,Created Date,Resolved Date\n")
